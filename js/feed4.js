...
const Feed = (() => {
  ...
  const _cardPost = (p, likeCount, cmtCount, liked) => {
    ...
    const img = p.video_url
      ? `<div class="card-img-wrap" style="border-radius:10px;overflow:hidden;border:0.5px solid #e4e6eb;position:relative;cursor:pointer;" onclick="openMedia('${Utils.esc(p.video_url)}','video')"><video id="vid-${p.id}" src="${Utils.esc(p.video_url)}" autoplay muted loop playsinline preload="auto" style="width:100%;height:auto;display:block;max-height:500px;object-fit:cover;" onended="this.currentTime=0;this.play()" oncanplay="this.muted=true;this.play()"></video><button onclick="event.stopPropagation();var v=document.getElementById('vid-${p.id}');v.muted=!v.muted;this.textContent=v.muted?'🔇':'🔊'" style="position:absolute;bottom:10px;right:10px;background:rgba(0,0,0,.5);border:none;border-radius:50%;width:32px;height:32px;font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;z-index:2;">🔇</button></div>`
    ...
    <article id="card-${p.id}" style="padding:14px 16px 0;background:#fff;">
      <div style="display:flex;gap:10px;">
        ...
        <div style="flex:1;min-width:0;padding-bottom:14px;">
          ...
          ${img ? img.replace('class="card-img-wrap"', 'style="border-radius:10px;overflow:hidden;margin-bottom:10px;"').replace('max-height:500px;object-fit:cover;', 'max-height:400px;object-fit:cover;') : ''}
