...
const Feed = (() => {
  ...
  const _cardPost = (p, likeCount, cmtCount, liked) => {
    ...
    const img = p.video_url
      ? `<div class="card-img-wrap" style="border-radius:10px;overflow:hidden;border:0.5px solid #e4e6eb;"><video src="${Utils.esc(p.video_url)}" playsinline controls preload="auto" style="width:100%;height:auto;display:block;max-height:600px;background:#000;"></video></div>`
      : (p.photo_url ? `<div class="card-img-wrap" style="border-radius:10px;overflow:hidden;border:0.5px solid #e4e6eb;"><img src="${Utils.esc(p.photo_url)}" loading="lazy" style="width:100%;height:auto;display:block;min-height:150px;max-height:600px;object-fit:cover;"></div>` : '')
