...
const Feed = (() => {
  ...
  const _card = (p, likeCount, cmtCount, liked, isMine) => {
    ...
    <article class="card" id="card-${p.id}">
      ...
      ${p.video_url
        ? `<div class="card-img-wrap"><video class="card-img" src="${Utils.esc(p.video_url)}" playsinline controls preload="metadata" style="max-height:500px;background:#000;"></video></div>`
        : p.photo_url
        ? `<div class="card-img-wrap"><img class="card-img" src="${Utils.esc(p.photo_url)}" loading="lazy" alt=""></div>`
        : ''
      }
  ...
  const _injectPromo = () => {
    ...
    if (promo.type === 'annonce') {
      ...
      const a = promo.data
      _el.insertAdjacentHTML('beforeend', `
      <article class="card card-promo" onclick="location.href='actus2.html'" class="card-clickable card-promo">
        <div class="card-head">
          <div class="c-av c-av-40" class="c-av c-av-40 card-av-actu">📰</div>
          <div class="card-meta">
            <div class="badge-actu">ACTU · ${Utils.esc(a.categorie || 'Quartier')}</div>
            <div class="card-meta-author">${Utils.esc(Utils.timeAgo ? Utils.timeAgo(a.date_publication) : '')}</div>
          </div>
